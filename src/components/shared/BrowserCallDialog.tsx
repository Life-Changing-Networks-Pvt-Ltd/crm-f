...
export function BrowserCallDialog() {
  ...
  const active = ["connecting", "ringing", "in-call"].includes(phone.status)
    ...
    <DialogContent className="max-w-sm">
      <DialogHeader className="items-center text-center">
        <div className={`mb-2 flex h-16 w-16 items-center justify-center rounded-full ${phone.status === "in-call" ? "bg-green-100 text-green-700" : phone.status === "error" ? "bg-destructive/10 text-destructive" : "bg-primary/10 text-primary"}`}>
          {phone.status === "connecting" || phone.status === "ringing" ? <Loader2 className="h-8 w-8 animate-spin" /> : phone.status === "error" ? <PhoneOff className="h-8 w-8" /> : <PhoneCall className="h-8 w-8" />}
        </div>
      ...
      <div className="mt-4 flex justify-center gap-3">
        ...
        {active ? <Button variant="destructive" size="icon" className="h-12 w-12 rounded-full" onClick={() => browserPhone.hangup()}><PhoneOff /></Button> : <>{phone.callLogId && <Button variant="outline" onClick={() => { const callLogId = phone.callLogId; browserPhone.dismiss(); navigate({ to: `/calls/${callLogId}` }) }}>View details</Button>}<Button onClick={() => browserPhone.dismiss()}><X className="mr-2 h-4 w-4" />Close</Button></>}
      </div>
    </DialogContent>
  </Dialog>
}
