...
import { ArrowLeft, Clock, FileText, Loader2, PhoneIncoming, RotateCcw, User } from "lucide-react"
...
export default function CallDetails() {
  ...
  const canRetryTranscript = can(currentUser, "calls.manage")
  ...
  const retryTranscript = async () => {
    setRetrying(true)
    try {
      await api.post(`/telephony/calls/${id}/transcription/retry`)
      setCall((current: any) => current ? { ...current, transcriptText: "", transcriptionStatus: "processing", transcriptionError: "" } : current)
      toast.success("Transcript generation restarted")
    } catch (error: any) {
      toast.error(error.response?.data?.message || "Could not retry the transcript")
    } finally {
      setRetrying(false)
    }
  }
  ...
  return <div className="flex flex-col gap-6 pb-8">
    ...
    <div className="grid gap-6 lg:grid-cols-3"><div className="space-y-6 lg:col-span-2"><Card><CardHeader><CardTitle>Voice recording</CardTitle><CardDescription>Stereo recording of the connected browser and lead conversation.</CardDescription></CardHeader><CardContent>{call.recordingUrl ? <audio src={call.recordingUrl} controls className="w-full" /> : <div className="rounded-md border border-dashed p-8 text-center text-sm text-muted-foreground">{call.recordingStatus === "failed" ? "Recording failed or was not generated." : "Recording is processing. Refresh after the provider callback arrives."}</div>}</CardContent></Card><Card><CardHeader><CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5" />Full transcript</CardTitle></CardHeader><CardContent>{usableTranscript(call.transcriptText) ? <div className="whitespace-pre-wrap rounded-lg bg-muted/40 p-5 text-sm leading-7">{call.transcriptText}</div> : call.transcriptionStatus === "failed" ? <div className="space-y-4 rounded-md border border-dashed p-8 text-center text-sm text-muted-foreground"><p>{`Transcription failed: ${call.transcriptionError || "The recording could not be transcribed."}`}</p>{canRetryTranscript && <Button variant="outline" onClick={retryTranscript} disabled={retrying}>{retrying ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <RotateCcw className="mr-2 h-4 w-4" />}Retry Transcript</Button>}</div> : <div className="rounded-md border border-dashed p-8 text-center text-sm text-muted-foreground">{call.recordingStatus === "ready" ? "Sellerslogin is generating the transcript. This page will update automatically." : "Transcript will be generated after the recording is ready."}</div>}</CardContent></Card></div><div className="space-y-6"><Card><CardHeader><CardTitle>Call information</CardTitle></CardHeader><CardContent className="grid gap-4"><Detail label="From virtual number" value={call.fromNumber} /><Detail label="Lead number" value={call.toNumber} /><Detail label="Lead type" value={call.leadModel} /><Detail label="Provider Call UUID" value={call.providerCallId} /><Detail label="Recording ID" value={call.recordingId} /><Detail label="Recording format" value={call.recordingFormat} /><Detail label="Hangup cause" value={call.hangupCause} /><Detail label="Hangup code" value={call.hangupCauseCode} /><Detail label="Created" value={new Date(call.createdAt).toLocaleString()} /><Detail label="Ended" value={call.endedAt ? new Date(call.endedAt).toLocaleString() : "—"} /></CardContent></Card>{call.manualComment && <Card><CardHeader><CardTitle>Agent notes</CardTitle></CardHeader><CardContent><p className="whitespace-pre-wrap text-sm">{call.manualComment}</p></CardContent></Card>}</div></div>
  </div>
}
