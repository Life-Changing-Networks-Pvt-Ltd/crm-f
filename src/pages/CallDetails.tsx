...
export default function CallDetails() {
  ...
  const load = async () => { try { const response = await api.get(`/telephony/calls/${id}`); setCall(response.data.data) } catch (error: any) { toast.error(error.response?.data?.message || "Call not found") } finally { setLoading(false) } }
  useEffect(() => { load() }, [id])
