...
export default function LeadManager() {
  ...
  const handlePageSelect = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSelectedPageId(pId || null);
  };

  const fetchForms = async () => {
    if (!selectedPageId) return [];
    try {
      // 1. Fetch forms for selected page from Facebook
      const fbRes = await api.get(`/facebook-marketing/pages/${selectedPageId}/forms`);
      const fbForms = fbRes.data?.data || [];
      
      // 2. Fetch automation status from local whatsapp-marketing backend
      const autoRes = await fetch("/api/automation-status", { credentials: "include" });
      const automationMap = await autoRes.json().catch(() => ({}));
      
      // 3. Merge them
      const mergedForms = fbForms.map((form: any) => ({
        id: form.id,
        name: form.name,
        status: form.status,
        assigned_template: automationMap[form.id]?.assigned_template || null,
  ...
  const { data: forms = [], isFetching: loadingForms } = useQuery({
    queryKey: ["forms", selectedPageId],
    queryFn: fetchForms,
  ...
  const triggerMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["forms", selectedPageId] });
    },
  });
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6 mb-6">
          <div className="flex flex-col md:flex-row gap-4 items-end">
            ...
            <button
              onClick={loadPages}
              disabled={loadingPages}
              className="bg-slate-100 text-slate-700 px-6 py-2 rounded-lg font-medium hover:bg-slate-200 transition flex items-center gap-2 h-[42px]"
            >
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          {!selectedPageId ? (
            <div className="rounded-xl border border-dashed border-slate-300 py-16 text-center">
              <Facebook className="mx-auto mb-3 text-slate-300" size={52} />
              <p className="text-lg text-slate-600">Select a Facebook Page to view forms</p>
            </div>
          ) : loadingForms ? (
            <div className="rounded-xl border border-dashed border-slate-300 py-16 text-center">
              <RefreshCw className="mx-auto mb-3 text-blue-500 animate-spin" size={52} />
              <p className="text-lg text-slate-600">Loading forms...</p>
            </div>
            ...
            <div className="rounded-xl border border-dashed border-slate-300 py-16 text-center">
              ...
              <p className="text-lg text-slate-600">No forms found for this page</p>
            </div>
          ) : (
