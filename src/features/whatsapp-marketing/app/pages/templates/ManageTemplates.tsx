...
import type { ButtonDef, Template } from "./type";
...
function normalizeListPayload(payload: any): Template[] {
...
export default function ManageTemplates() {
  ...
  const { data: templatesResponse, isLoading } = useQuery<any>({
    queryKey: ["/api/templates"],
    queryFn: async () => {
      const res = await fetchWithAuth("/api/templates?metaOnly=true");
      if (!res.ok) throw new Error("Failed to fetch templates");
      return res.json();
    },
  });
  const templates = useMemo(
    () => normalizeListPayload(templatesResponse),
    [templatesResponse]
  );

  const userDataString = typeof window !== "undefined"
    ? sessionStorage.getItem("whatsapp_auth_user")
    : null;

  useEffect(() => {
    setSelectedTemplateIds((prev) => {
      const validIds = new Set(templates.map((template) => template.id));
      const filteredPrev = [...prev].filter((id) => validIds.has(id));
      if (filteredPrev.length === prev.size) {
        return prev;
      }
      return new Set(filteredPrev);
    });
  }, [templates]);
  ...
  const syncMetaTemplatesMutation = useMutation({
    mutationFn: async () => {
      const res = await fetchWithAuth("/api/templates/sync-meta", {
        method: "POST",
      });
  ...
  const submitForApprovalMutation = useMutation({
    mutationFn: async (templateId: string) => {
      const res = await fetchWithAuth(`/api/templates/${templateId}/submit-approval`, {
        method: "POST",
      });
      const data = await res.json();
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetchWithAuth(
        `/api/templates/${id}?deleteFromMeta=true`,
        { method: "DELETE" }
      );
  ...
  const bulkDeleteMutation = useMutation({
    mutationFn: async (ids: string[]) => {
      const res = await fetchWithAuth(
        `/api/templates/bulk-delete?deleteFromMeta=true`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
