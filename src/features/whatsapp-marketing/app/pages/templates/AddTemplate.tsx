...
export default function AddTemplate() {
  ...
  const [authSecurityRecommendation, setAuthSecurityRecommendation] =
    useState(true);
  const [authCodeExpirationMinutes, setAuthCodeExpirationMinutes] =
    useState(10);
    ...
    onSuccess: async (template) => {
      ...
      if (autoSubmitAfterCreate && template?.id) {
        try {
          ...
          if (!submitRes.ok) {
            throw new Error(
              submitData.userMessage ||
                submitData.message ||
                submitData.error ||
                "Template created but submit failed"
            );
          }
  ...
  const handleSubmit = () => {
    ...
    if (!isAuthTemplate && !body.trim()) {
      toast.error("Please enter body text");
      return;
    }
    if (!isAuthTemplate && body.length > 1000) {
    ...
    createTemplateMutation.mutate({
      ...
      content: isAuthTemplate ? "" : body,
      footer: isAuthTemplate ? null : footer || null,
      buttons: validButtons.length > 0 ? validButtons : undefined,
      authAddSecurityRecommendation: isAuthTemplate
        ? authSecurityRecommendation
        : undefined,
      authCodeExpirationMinutes: isAuthTemplate
        ? authCodeExpirationMinutes
        : undefined,
      authOtpType: isAuthTemplate ? "COPY_CODE" : undefined,
      status: "pending",
    });
  };
          ...
          <div className="lg:col-span-2">
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                {!isAuthTemplate && <div className="grid gap-2">
                ...
                {!isAuthTemplate && headerType === "text" && (
                  <div className="grid gap-2">
                  ...
                  <div className="space-y-4 rounded-lg border border-blue-200 bg-blue-50/60 p-4">
                    <div>
                      <p className="font-medium text-slate-900">
                        Authentication message setup
                      </p>
                      <p className="mt-1 text-xs text-slate-600">
                        Meta generates the verification-code message body. Custom
                        body text, headers, footers, and custom buttons are not used.
                      </p>
                    </div>
                    ...
                    <label className="flex items-start gap-3 text-sm text-slate-700">
                      <input
                        ...
                        onChange={(event) =>
                          setAuthSecurityRecommendation(event.target.checked)
                        }
                      ...
                      <span>
                        ...
                        <span className="block text-xs text-slate-500">
                          Adds Meta's standard warning not to share the code.
                        </span>
                      </span>
                    </label>

                    <div className="grid gap-2 sm:max-w-xs">
                      <Label htmlFor="auth-expiry">Code expires in</Label>
                      <Select
                        value={String(authCodeExpirationMinutes)}
                        onValueChange={(value) =>
                          setAuthCodeExpirationMinutes(Number(value))
                        }
                      >
                    ...
                    <div className="rounded-md border bg-white px-3 py-2 text-sm">
                      <span className="font-medium">Code delivery:</span>{" "}
                      Copy code button
                    </div>
                  </div>
                )}
            ...
            <Card>
              ...
              <CardContent>
                <PhonePreview
                  ...
                  body={
                    isAuthTemplate
                      ? `123456 is your verification code.${authSecurityRecommendation ? " For your security, do not share this code." : ""}`
                      : body
                  }
                  footer={
                    isAuthTemplate
                      ? `Expires in ${authCodeExpirationMinutes} minutes.`
                      : footer
                  }
                  buttons={
                    isAuthTemplate
                      ? [{ id: "auth-copy-code", type: "quick_reply", text: "Copy code" }]
